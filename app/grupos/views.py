...
#DETALHAR
def detalhesEntidade(request, pk):
    entidade = get_object_or_404(Entidade, pk=pk)
    form = EntidadeForm(request.GET or None, instance=entidade)
    return render(request, 'grupos/detalhesEntidade.html', {'form':form, 'entidade':entidade})
...
#DETALHAR
def detalhesRepresentante(request, pk):
    representante = get_object_or_404(Representante, pk=pk)
    form = RepresentanteForm(request.GET or None, instance=representante)
    return render(request, 'grupos/detalhesRepresentante.html', {'form':form, 'representante':representante})
...
def cadastrarUsuario(request):
    ...
    try:
        if request.method == 'POST':
            ...
            senhaConfirmacao = request.POST.get('senhaConfirmacao')

            # Verificando se o usuário é administrador
            if request.user.is_superuser is False:
                erro = "Você não tem permissão para cadastrar usuários. Solicite a um usuário administrador."
                raise Exception(erro)

            # Se já existe usuário com mesmo user cadastrado
            if User.objects.filter(username=(usuario)).exists():
                erro = 'Já existe um usuário cadastrado com o usuário escolhido. Por favor escolha outro.'
                raise Exception(erro)

            # Verifica se as duas senhas digitadas coincidem
            if request.POST.get('senha') != request.POST.get('senhaConfirmacao'):
                erro = 'As senhas digitadas não coincidem. Certifique-se de que as duas senhas digitadas são iguais.'
                raise Exception(erro)
            ...
            if senha.find(" ") != -1 or senha == '':
                erro = 'A senha não pode conter espaço em branco. Utilize apenas letras, números e/ou caracteres especiais.'
                raise Exception(erro)
            
            primeiroNome = request.POST.get('primeiroNome') if request.POST.get('primeiroNome') else ""
            sobrenome = request.POST.get('sobrenome') if request.POST.get('sobrenome') else ""
            ...
            novoUsuario = User.objects.create_user(
                ...
                email = email,
                password = senha,
                is_superuser = seraAdministrador,
                is_staff = seraAdministrador,
                is_active = True
            )
            ...
            grupo, foiCriado = Group.objects.get_or_create(name='primeirologin')
            novoUsuario.groups.add(grupo)

            messages.success(request, "Usuário cadastrado com sucesso.", extra_tags='alert alert-success px-2')
            return redirect('Lista de Usuarios')

        else:
            return render(request, 'registration/cadastrarUsuario.html')
    except Exception as excecao:
